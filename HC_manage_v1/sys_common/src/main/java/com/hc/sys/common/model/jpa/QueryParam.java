package com.hc.sys.common.model.jpa;
...
public class QueryParam {
	...
	/** 管理后台分页查询方式-简单快速查询 */
	public static final byte PAGEDATALIST_SEARCHSTYLE_SIMPLE = 1;
	/** 管理后台分页查询方式-高级查询 */
	public static final byte PAGEDATALIST_SEARCHSTYLE_ADVANCED = 2;
	...
	public QueryParam addOrFilter(List<SearchFilter> list){
		for (SearchFilter searchFilter : list) {
			map.put(searchFilter.fieldName+"", searchFilter.value);
		}
		SearchFilter filter = new SearchFilter(null, Operators.OR, null);
	...
	public QueryParam addOrAndFilter(SearchFilter... value) {
		SearchFilter orFilter = null;
		for (int i = 0; i < value.length; i++) {
			if (i == 0) {
				if (value[i].rightFieldName != null) {
					SearchFilter leftFilter = new SearchFilter(value[i].fieldName, value[i].operator, value[i].value);
					SearchFilter rightFilter = new SearchFilter(value[i].rightFieldName, value[i].rightOperator, value[i].rightValue);
					orFilter = new SearchFilter(leftFilter, Operators.AND, rightFilter);
				} else {
					orFilter = value[0];;
				}
			} else if (value[i].rightFieldName != null){
				SearchFilter leftFilter = new SearchFilter(value[i].fieldName, value[i].operator, value[i].value);
				SearchFilter rightFilter = new SearchFilter(value[i].rightFieldName, value[i].rightOperator, value[i].rightValue);
				SearchFilter filter = new SearchFilter(leftFilter, Operators.AND, rightFilter);
				orFilter = new SearchFilter(orFilter, Operators.OR, filter);
			} else {
				orFilter = new SearchFilter(orFilter, Operators.OR, value[i]);
			}
		}
		...
		switch (filter.operator) {
			...
			case OR:
				try {
					...
					LogUtil.error(e.getMessage()+"  "+e.getCause().getMessage());
					expression = parseExpression(StringUtil.isNull(filter.fieldName), root);
					p = builder.equal(expression, filter.value);
				}
				break;
