package com.hc.sys.common.dao.jpa;
...
import com.hc.sys.common.util.reflect.ReflectUtil;
...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	public boolean update(Collection<T> ts) {
		try {
			...
			LogUtil.error(message+"   "+e.getMessage()+"  "+e.getStackTrace());
		}
		return false;
	}
	...
	public T save(T entity) {
		try {
			...
			LogUtil.error("数据库保存错误！"+e.getMessage());
	...
	public void delete(Serializable[] entityids) {
		for (Object id : entityids){
			try {
				...
				LogUtil.error("数据库删除数据失败！"+e.getMessage());
			}
		}
	...
	public T find(Serializable entityId) {
		T t=null;
		if (entityId == null)
			LogUtil.error(this.entityClass.getName() + "  传入的实体id不能为空");
			Result.error(this.entityClass.getName() + ":传入的实体id不能为空");
			try {
				 t=em.find(this.entityClass, entityId);
				return t;
	...
	public <G> List<G> listBySql(String sql, String[] names, Object[] values, Class<G> clazz) {
		...
		for (int n = 0; n < rows.size(); n++) {
			...
			try {
				...
				LogUtil.error(clazz.getName() + " instance fail.");
			}
			if (o == null)
				return null;
	...
	@SuppressWarnings("unchecked")
	@Override
	public <G> G findByJpql(String jpql, String[] names, Object[] values) {
		Query query = em.createQuery(jpql);
		for (int i = 0; i < names.length; i++) {
			query.setParameter(names[i], values[i]);
		}
